...
MQTTConnection::MQTTConnection(MQTTConnection &&other)
		: Connection(std::move(other)), mosq(other.mosq), mqttParameters(std::move(other.mqttParameters)),
...
void MQTTConnection::connect() {
	...
	if (mqttParameters.tls) {
		ret = mosquitto_tls_set(mosq, mqttParameters.cafile.empty() ? nullptr : mqttParameters.cafile.c_str(),
														...
														mqttParameters.keyfile.empty() ? nullptr : mqttParameters.keyfile.c_str(), nullptr);
		if (ret) {
			status = CONNECTION_STATUS_ERROR;
...
bool MQTTConnection::send(const Message &message) {
	...
	mosquitto_publish(mosq, NULL, mqtt_message->topic.c_str(), mqtt_message->payload.size(),
										mqtt_message->payload.c_str(), mqtt_message->qos, mqtt_message->retain);
	queueSize++;
	return true;
}
...
void MQTTConnection::subscribe(const std::string &topic) { mosquitto_subscribe(mosq, NULL, topic.c_str(), 0); }

void MQTTConnection::unsubscribe(const std::string &topic) { mosquitto_unsubscribe(mosq, NULL, topic.c_str()); }
